...
export interface OrbSceneApi {
  ...
  zoomOut(): void;
  resetView(): void;
  /** Feed a 0..1 audio level to make the orb glow pulse while ELISA speaks. */
...
export function createOrbScene(container: HTMLElement): OrbSceneApi {
  ...
  const chromaticShader = {
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        gl_FragColor = vec4(cr.r * 0.6, cg.g * 1.1, cb.b * 0.65, 1.0) * flicker;
        // Push towards blue/black tone
        gl_FragColor.rgb = mix(gl_FragColor.rgb, gl_FragColor.rgb * vec3(0.65, 0.9, 1.25), 0.3);
      }
    `,
  };
  ...
  // ——— COLORS ———
  const C_BRIGHT = 0x38a8ff;
  const C_MID = 0x168fe8;
  const C_DIM = 0x0b5f9e;
  const C_FAINT = 0x062844;
  const C_HOT = 0xa8dcff;
  ...
  function makeTextSprite(text: string, size = 0.08) {
    ...
    ctx.fillStyle = `rgba(${(30 + Math.random() * 60) | 0}, 255, ${(90 + Math.random() * 60) | 0}, ${alpha})`;
  ...
  g.addColorStop(0, "rgba(48,255,106,1)");
  g.addColorStop(0.2, "rgba(20,220,90,0.6)");
  g.addColorStop(0.5, "rgba(0,150,60,0.15)");
  g.addColorStop(1, "rgba(0,70,30,0)");
  dCtx.fillStyle = g;
  ...
  let rafId = 0;
  let disposed = false;
  let voiceLevel = 0; // 0..1, driven externally while ELISA is speaking
  ...
  function animate() {
    ...
    // Bloom pulse — base idle shimmer plus a boost while ELISA is speaking
    bloom.strength = 1.6 + Math.sin(t * 0.8) * 0.3 + voiceLevel * 1.4;
  ...
  return {
    ...
    setVoiceLevel: (level: number) => {
      voiceLevel = Math.min(1, Math.max(0, level));
    },
    dispose,
  };
}
